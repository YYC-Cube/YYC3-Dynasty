/**
 * file: SessionsPanel.tsx
 * description: 小任务面板 · Agent 会话列表
 * author: YanYuCloudCube Team
 * version: v1.0.0
 * created: 2026-03-21
 * updated: 2026-07-12
 * status: active
 * tags: [component],[sessions],[tasks],[agent]
 *
 * brief: Agent 小任务/会话列表面板
 *
 * details:
 * - 展示所有 Agent 会话
 * - 按状态筛选
...
export default function SessionsPanel() {
  ...
  if (sessFilter === 'active')
    filtered = sessions.filter((t) => !['Done', 'Cancelled'].includes(t.state));
              ...
              <div className="sess-card" key={t.id} onClick={() => setDetailTask(t)}>
                <div className="sc-top">
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span className={`tag st-${st}`} style={{ fontSize: 10 }}>
                      {STATE_LABEL[st] || st}
                    </span>
                  </div>
                </div>
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 8 }}>
            📋 最近活动{' '}
            <span style={{ fontWeight: 400, color: 'var(--muted)' }}>({acts.length} 条)</span>
          </div>
          <div
            style={{
              maxHeight: 350,
          ...
          >
            {!acts.length ? (
              <div
                style={{ padding: 16, color: 'var(--muted)', fontSize: 12, textAlign: 'center' }}
              >
                暂无活动记录
              </div>
            ) : (
