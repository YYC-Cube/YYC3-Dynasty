/**
 * file: MorningPanel.tsx
 * description: 天下要闻面板 · 早报与配置管理
 * author: YanYuCloudCube Team
 * version: v1.0.0
 * created: 2026-03-21
 * updated: 2026-07-12
 * status: active
 * tags: [component],[news],[morning],[config]
 *
 * brief: 天下要闻/早报面板，支持分类订阅配置
 *
 * details:
 * - 展示各类新闻资讯
 * - 分类配置管理
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--muted)' }}>
            {dateStr && `${dateStr} | `}
            {morningBrief?.generated_at && `采集于 ${morningBrief.generated_at} | `}共 {totalNews}{' '}
            条要闻
          </div>
        </div>
            ...
            return (
              <div className="mb-cat" key={cat}>
                <div className="mb-cat-hdr">
                  ...
                  <span className="mb-cat-name" style={{ color: meta.color }}>
                    {cat}
                  </span>
                ...
                <div className="mb-news-list">
                  {!scored.length ? (
                    <div className="mb-empty" style={{ padding: 16 }}>
                      暂无新闻
                    </div>
                  ) : (
                    scored.map((item, i) => {
    ...
    >
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 6 }}>
          {(config.keywords || []).map((kw, i) => (
            ...
            >
              {kw}
              <span
                style={{ cursor: 'pointer', marginLeft: 4, color: 'var(--danger)' }}
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        {(config.custom_feeds || []).map((f, i) => (
          <div
            key={i}
            style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4, fontSize: 11 }}
          >
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
          ...
          <button
            className="btn btn-g"
            onClick={() => {
              onAddFeed(feedName, feedUrl, feedCat);
              setFeedName('');
              setFeedUrl('');
            }}
            style={{ fontSize: 11, padding: '4px 12px' }}
