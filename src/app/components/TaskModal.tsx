/**
 * file: TaskModal.tsx
 * description: 任务详情弹窗 · 实时活动数据展示
 * author: YanYuCloudCube Team
 * version: v1.0.0
 * created: 2026-03-21
 * updated: 2026-07-12
 * status: active
 * tags: [component],[modal],[task],[detail]
 *
 * brief: 任务/旨意详情弹窗，展示实时活动数据
 *
 * details:
 * - 任务活动流实时数据
 * - Agent 调度状态
...
import type { ActivityEntry, SchedulerStateData, TaskActivityData, TodoItem } from '../api';
import { api } from '../api';
...
export default function TaskModal() {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- task?.state narrows intentionally; full task object would over-trigger
  ...
  const doSchedAction = async (action: string) => {
    ...
    const handlers: Record<
      string,
      (id: string, r: string) => Promise<{ ok: boolean; message?: string; error?: string }>
    > = {
    ...
    <div className="modal-bg open" onClick={close}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          {activeStage && (
            <div className="cur-stage">
              ...
              <div className="cs-info">
                <div className="cs-dept" style={{ color: deptColor(activeStage.dept) }}>
                  {activeStage.dept}
                </div>
            ...
            {stages.map((s, i) => (
              <div className="mp-stage" key={s.key}>
                ...
                {i < stages.length - 1 && (
                  <div
                    className="mp-arrow"
                    style={s.status === 'done' ? { color: 'var(--ok)', opacity: 0.6 } : {}}
                  >
                    →
                  </div>
                )}
          ...
          <div className="task-actions">
            ...
            {canResume && (
              <button
                ...
                onClick={() => doTaskAction('resume', '恢复执行')}
              >
                ▶️ 恢复执行
              </button>
            )}
            {['Review', 'Menxia'].includes(task.state) && (
              <>
                <button className="btn btn-success btn-sm" onClick={() => doReview('approve')}>
                  ✅ 准奏
                </button>
                <button className="btn btn-danger btn-sm" onClick={() => doReview('reject')}>
                  🚫 封驳
                </button>
              </>
            )}
            {[
              ...
              <button className="btn btn-primary btn-sm" onClick={doAdvance}>
                ⏩ 推进到下一步
              </button>
            )}
          </div>
          ...
          <div className="sched-section">
            <div className="sched-head">
              ...
              <span className="sched-status">
                {sched
                  ? `${sched.enabled === false ? '已禁用' : '运行中'} · 阈值 ${sched.stallThresholdSec || 180}s`
                  : '加载中...'}
              </span>
            </div>
            <div className="sched-grid">
              <div className="sched-kpi">
                <div className="k">停滞时长</div>
                <div className="v">{fmtStalled(stalledSec)}</div>
              </div>
              <div className="sched-kpi">
                <div className="k">重试次数</div>
                <div className="v">{sched?.retryCount || 0}</div>
              </div>
              <div className="sched-kpi">
                <div className="k">升级级别</div>
                <div className="v">
                  {!sched?.escalationLevel
                    ? '无'
              ...
              <div className="sched-kpi">
                <div className="k">派发状态</div>
                <div className="v">{sched?.lastDispatchStatus || 'idle'}</div>
              </div>
            </div>
            {sched && (
              <div className="sched-line">
                {sched.lastProgressAt && (
                  <span>
                    最近进展 {(sched.lastProgressAt || '').replace('T', ' ').substring(0, 19)}
                  </span>
                )}
                {sched.lastDispatchAt && (
                  <span>
                    最近派发 {(sched.lastDispatchAt || '').replace('T', ' ').substring(0, 19)}
                  </span>
                )}
            ...
            <div className="sched-actions">
              <button className="sched-btn" onClick={() => doSchedAction('retry')}>
                🔁 重试派发
              </button>
              <button className="sched-btn warn" onClick={() => doSchedAction('escalate')}>
                📣 升级协调
              </button>
              <button className="sched-btn danger" onClick={() => doSchedAction('rollback')}>
                ↩️ 回滚稳定点
              </button>
              <button className="sched-btn" onClick={() => doSchedAction('scan')}>
                🔍 立即扫描
              </button>
            </div>
          </div>
          ...
          {/* Todo List */}
          {todoTotal > 0 && <TodoSection todos={todos} todoDone={todoDone} todoTotal={todoTotal} />}
          ...
          <div className="m-section">
            <div className="m-rows">
              ...
              <div className="m-row">
                ...
                <div className="mr-val">
                  <span className={`tag dt-${(task.org || '').replace(/\s/g, '')}`}>
                    {task.org || '—'}
                  </span>
                </div>
              </div>
              {task.eta && task.eta !== '-' && (
                <div className="m-row">
                  <div className="mr-label">预计完成</div>
                  <div className="mr-val">{task.eta}</div>
                </div>
              )}
              {task.block && task.block !== '无' && task.block !== '-' && (
                <div className="m-row">
                  <div className="mr-label" style={{ color: 'var(--danger)' }}>
                    阻塞项
                  </div>
                  <div className="mr-val" style={{ color: 'var(--danger)' }}>
                    {task.block}
                  </div>
                </div>
              )}
              {task.now && task.now !== '-' && (
                <div className="m-row" style={{ gridColumn: '1/-1' }}>
                  ...
                  <div className="mr-val" style={{ fontWeight: 400, fontSize: 12 }}>
                    <MarkdownRenderer content={task.now} compact />
                  </div>
                </div>
              )}
              {task.ac && (
                    ...
                    <div className="fl-item" key={i}>
                      ...
                      <div className="fl-content">
                        <div className="fl-who">
                          <span className="from" style={{ color: col }}>
                            {fl.from}
                          </span>
                          <span style={{ color: 'var(--muted)' }}> → </span>
                          <span className="to" style={{ color: deptColor(fl.to || '') }}>
                            {fl.to}
                          </span>
                        </div>
          ...
          <LiveActivitySection
            data={activityData}
            isDone={['Done', 'Cancelled'].includes(task.state)}
            logRef={logRef}
          />
        </div>
      </div>
...
function LiveActivitySection({
  ...
  if (data.relatedAgents && data.relatedAgents.length > 1)
    agentParts.push(`${data.relatedAgents.length}个 Agent`);
      ...
      {ts && (
        <div style={{ padding: '4px 0 8px', borderBottom: '1px solid var(--line)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            ...
            <span
              style={{
                fontSize: 20,
                fontWeight: 700,
                color: ts.percent >= 100 ? '#22c55e' : ts.percent >= 50 ? '#60a5fa' : 'var(--text)',
              }}
            >
              {ts.percent}%
            </span>
            <span style={{ fontSize: 10, color: 'var(--muted)' }}>
              ✅{ts.completed} 🔄{ts.inProgress} ⬜{ts.notStarted} / 共{ts.total}项
            </span>
          </div>
          <div
            style={{
          ...
          >
            <div
              style={{
                width: `${ts.total ? (ts.completed / ts.total) * 100 : 0}%`,
                background: '#22c55e',
                transition: 'width .3s',
              }}
            />
            <div
              style={{
                width: `${ts.total ? (ts.inProgress / ts.total) * 100 : 0}%`,
                background: '#3b82f6',
                transition: 'width .3s',
              }}
            />
        ...
        >
          ...
          {rs.totalTokens != null && (
            <span style={{ fontSize: 11, color: 'var(--muted)' }}>
              🔢 {rs.totalTokens.toLocaleString()} tokens
            </span>
          )}
          {rs.totalCost != null && (
            <span style={{ fontSize: 11, color: 'var(--muted)' }}>
              💰 ${rs.totalCost.toFixed(4)}
            </span>
          )}
          {rs.totalElapsedSec != null && (
            <span style={{ fontSize: 11, color: 'var(--muted)' }}>
              ⏳ {rs.totalElapsedSec >= 60 ? `${Math.floor(rs.totalElapsedSec / 60)}分` : ''}
              {rs.totalElapsedSec % 60}秒
            </span>
          )}
        </div>
      )}
...
function ActivityEntryView({ entry: a }: { entry: ActivityEntry }) {
  ...
  if (a.kind === 'todos') {
    ...
    if (a.diff) {
      (a.diff.changed || []).forEach((c) =>
        diffMap.set(c.id, { type: 'changed', from: c.from, to: c.to }),
      );
        ...
        {items.map((td) => {
          ...
          return (
            <div key={td.id} style={style}>
              {icon} {td.title}
              {d && d.type === 'changed' && d.to === 'completed' && (
                <span style={{ color: '#22c55e', fontSize: 9, marginLeft: 4 }}>✨刚完成</span>
              )}
              {d && d.type === 'changed' && d.to !== 'completed' && (
              ...
              {d && d.type === 'added' && (
                <span style={{ color: '#3b82f6', fontSize: 9, marginLeft: 4 }}>🆕新增</span>
              )}
            </div>
          );
        })}
        {a.diff?.removed?.map((r) => (
          <div key={r.id} style={{ opacity: 0.4, textDecoration: 'line-through' }}>
            🗑 {r.title}
          </div>
        ))}
      </div>
        ...
        {a.tools?.map((tc, i) => (
          <div className="la-entry la-tool" key={i}>
            ...
            <span className="la-body">
              ...
              <span className="la-trunc">{tc.input_preview || ''}</span>
            </span>
            <span className="la-time">{time}</span>
          </div>
        ))}
  ...
  if (a.kind === 'tool_result') {
    ...
    return (
      <div className={`la-entry la-tool-result ${ok ? 'ok' : 'err'}`}>
        ...
        <span className="la-body">
          {agBadge}
          <span className="la-tool-name">{a.tool || ''}</span>
          {a.output ? a.output.substring(0, 150) : ''}
        </span>
        <span className="la-time">{time}</span>
      </div>
    );
