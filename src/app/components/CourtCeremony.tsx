/**
 * file: CourtCeremony.tsx
 * description: 早朝仪式组件 · 每日首次打开时展示
 * author: YanYuCloudCube Team
 * version: v1.0.0
 * created: 2026-03-21
 * updated: 2026-07-12
 * status: active
 * tags: [component],[ceremony],[ui]
 *
 * brief: 早朝开场仪式动画，每日首次打开应用时展示
 *
 * details:
 * - 每日首次打开展示，持续 3.5s 自动跳过
 * - 显示待办/已完成/超期任务统计
 * - 点击任意处跳过动画
 *
 * dependencies: React, ../store
 * exports: CourtCeremony (default)
 */
...
export default function CourtCeremony() {
  ...
  const overdue = jjc.filter(
    (t) =>
      ...
      t.state !== 'Cancelled' &&
      t.eta &&
      new Date(t.eta.replace(' ', 'T')) < new Date(),
  ).length;
