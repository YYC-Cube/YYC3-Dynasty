 ...
 * description: 省部调度面板 · Agent 运行监控
 * author: YanYuCloudCube Team
 * version: v1.0.0
 * created: 2026-03-21
 * updated: 2026-07-12
 * status: active
 * tags: [component],[monitor],[agent],[dashboard]
 *
 * brief: 省部调度 — Agent 运行状态监控面板
 *
 * details:
 * - 各 Agent 在线/离线状态
 * - 网关状态监控
...
export default function MonitorPanel() {
  ...
  if (officialsData?.officials) {
    officialsData.officials.forEach((o) => {
      offMap[o.id] = o;
    });
  }
      ...
      {asData && asData.ok && (
        <div className="as-panel">
          <div className="as-header">
            ...
            {offline + unconf > 0 && (
              <button
                ...
                style={{ marginLeft: 4, borderColor: 'var(--warn)', color: 'var(--warn)' }}
              >
                ⚡ 全部唤醒
              </button>
            )}
          ...
          <div className="as-summary">
            <span>
              <span
                className="as-dot running"
                style={{ position: 'static', width: 8, height: 8 }}
              />{' '}
              {running} 运行中
            </span>
            <span>
              <span className="as-dot idle" style={{ position: 'static', width: 8, height: 8 }} />{' '}
              {idle} 待命
            </span>
            {offline > 0 && (
              <span>
                <span
                  className="as-dot offline"
                  style={{ position: 'static', width: 8, height: 8 }}
                />{' '}
                {offline} 离线
              </span>
                    ...
                    <div key={t.id} className="dc-task" onClick={() => setModalTaskId(t.id)}>
                      ...
                      <div className="dc-task-meta">
                        ...
                        {t.block && t.block !== '无' && (
                          <span
                            className="tag"
                            style={{ borderColor: '#ff527044', color: 'var(--danger)' }}
                          >
                            🚫{t.block}
                          </span>
                        )}
