/**
 * file: SkillsConfig.tsx
 * description: 技能配置组件 · AI Agent 技能管理与远程导入
 * author: YanYuCloudCube Team
 * version: v1.0.0
 * created: 2026-03-21
 * updated: 2026-07-12
 * status: active
 * tags: [component],[config],[skills],[ai]
 *
 * brief: AI Agent 技能管理，支持本地与远程技能源
 *
 * details:
 * - 各 Agent 的技能列表管理
 * - 远程技能源导入
 * - 社区技能快选
 *
 * dependencies: React, ../store, ../api
 * exports: SkillsConfig
 */

import { useEffect, useState } from 'react';
import { useStore } from '../store';
import { api, type RemoteSkillItem } from '../api';
import { COMMUNITY_SOURCES } from '../domain/skills';

// 社区技能源 — 从 domain/skills.ts 导入（权威定义）
// 包含 obra/superpowers、anthropics/skills、ComposioHQ/awesome-claude-skills
// 每个技能含 name + raw SKILL.md URL，供社区一键导入
...
export default function SkillsConfig() {
  ...
  const [quickPickSource, setQuickPickSource] = useState<(typeof COMMUNITY_SOURCES)[0] | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- loadRemoteSkills is stable-enough for this one-shot trigger
  }, [activeTab]);
    ...
    <div>
      ...
      >
        <button
          ...
          onClick={() => {
            setAddRemoteForm(true);
            setQuickPickSource(null);
          }}
        >
          ＋ 添加远程 Skill
        </button>
          ...
          >
            ...
            >
              {quickPickSource.skills.map((sk) => {
                const alreadyAdded = remoteSkills.some(
                  (r) => r.skillName === sk.name && r.agentId === quickPickAgent,
                );
                return (
                  <div
                    key={sk.name}
                  ...
                  >
                    <div>
                      ...
                      >
                        {sk.url.split('/').slice(-2).join('/')}
                      </div>
                    </div>
                    {alreadyAdded ? (
                      <span style={{ fontSize: 10, color: '#4caf88', fontWeight: 600 }}>
                        ✓ 已导入
                      </span>
                    ) : (
                      <button
                ...
                <div>
                  ...
                  >
                    <span>
                      ...
                      >
                        {sk.sourceUrl.length > 60 ? sk.sourceUrl.slice(0, 60) + '…' : sk.sourceUrl}
                      </a>
                    </span>
                    <span>
    ...
    <div>
        ...
        {[
          {
            key: 'local',
            label: '🏛️ 本地技能',
            count: agentConfig.agents.reduce((n, a) => n + (a.skills?.length || 0), 0),
          },
          ...
          <div
            ...
            style={{
              ...
              borderBottom:
                activeTab === t.key ? '1px solid var(--panel)' : '1px solid transparent',
              position: 'relative',
              bottom: -1,
              transition: 'all .15s',
            }}
      ...
      {skillModal && (
        <div className="modal-bg open" onClick={() => setSkillModal(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <button className="modal-close" onClick={() => setSkillModal(null)}>
              ✕
            </button>
            <div className="modal-body">
              ...
              <div className="sk-modal-body">
                <div
                  className="sk-md"
                  style={{ whiteSpace: 'pre-wrap', fontSize: 12, lineHeight: 1.7 }}
                >
                  {skillModal.content}
                </div>
                {skillModal.path && (
                  <div
                    className="sk-path"
                    style={{ fontSize: 10, color: 'var(--muted)', marginTop: 12 }}
                  >
                    📂 {skillModal.path}
                  </div>
                )}
      ...
      {addForm && (
        <div className="modal-bg open" onClick={() => setAddForm(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <button className="modal-close" onClick={() => setAddForm(null)}>
              ✕
            </button>
            <div className="modal-body">
              <div
              ...
              >
                ...
                <br />• 技能名称使用<b style={{ color: 'var(--text)' }}>小写英文 + 连字符</b>
                <br />
                • 创建后会生成模板文件 SKILL.md
                <br />• 技能会在 agent 收到相关任务时
                <b style={{ color: 'var(--text)' }}>自动激活</b>
              </div>
      ...
      {addRemoteForm && (
        <div className="modal-bg open" onClick={() => setAddRemoteForm(false)}>
          <div className="modal" style={{ maxWidth: 520 }} onClick={(e) => e.stopPropagation()}>
            <button className="modal-close" onClick={() => setAddRemoteForm(false)}>
              ✕
            </button>
            <div className="modal-body">
              <div
              ...
              >
                <div>
                  <label
                    style={{ fontSize: 12, fontWeight: 600, display: 'block', marginBottom: 6 }}
                  >
                    目标 Agent <span style={{ color: '#ff5270' }}>*</span>
                  </label>
                  <select
                    required
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setRemoteFormData((p) => ({
                        ...p,
                        skillName: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ''),
                      }))
                    }
                    style={{
                      width: '100%',
                ...
                <div>
                  <label
                    style={{ fontSize: 12, fontWeight: 600, display: 'block', marginBottom: 6 }}
                  >
                    源 URL <span style={{ color: '#ff5270' }}>*</span>
                  </label>
                  <input
                    type="url"
                    required
                    ...
                    onChange={(e) =>
                      setRemoteFormData((p) => ({ ...p, sourceUrl: e.target.value }))
                    }
                    style={{
                      width: '100%',
                      padding: '10px 12px',
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setRemoteFormData((p) => ({ ...p, description: e.target.value }))
                    }
                    style={{
                      width: '100%',
                      padding: '10px 12px',
                ...
                <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 4 }}>
                  <button
                    type="button"
                    className="btn btn-g"
                    onClick={() => setAddRemoteForm(false)}
                    style={{ padding: '8px 20px' }}
                  >
                    取消
                  </button>
