 ...
 * description: 模型配置组件 · AI 模型管理与切换
 * author: YanYuCloudCube Team
 * version: v1.0.0
 * created: 2026-03-21
 * updated: 2026-07-12
 * status: active
 * tags: [component],[config],[ai],[models]
 *
 * brief: 各 Agent 的 AI 模型配置与管理
 *
 * details:
 * - 为各 Agent 分配/切换 AI 模型
 * - 显示模型变更日志
 * - 调度渠道配置
 *
  ...
  const applyModel = async (agentId: string) => {
    ...
    try {
      ...
      if (r.ok) {
        setStatusMap((p) => ({
          ...p,
          [agentId]: { cls: 'ok', text: '✅ 已提交，Gateway 重启中（约5秒）' },
        }));
        ...
        setStatusMap((p) => ({
          ...p,
          [agentId]: { cls: 'err', text: '❌ ' + (r.error || '错误') },
        }));
      }
    } catch {
              ...
              <div className="mc-top">
                ...
                <div>
                  <div className="mc-name">
                    {ag.label} <span style={{ fontSize: 11, color: 'var(--muted)' }}>{ag.id}</span>
                  </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0' }}>
          ...
          {channelStatus && (
            <span
              style={{
                fontSize: 12,
                color: channelStatus.startsWith('✅') ? 'var(--success)' : 'var(--danger)',
              }}
            >
              {channelStatus}
            </span>
