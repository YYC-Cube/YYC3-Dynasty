/**
 * file: store.ts
 * description: 全局状态管理 · 基于 Zustand 的三省六部看板状态
 * author: YanYuCloudCube Team
 * version: v1.0.0
 * created: 2026-03-21
 * updated: 2026-07-12
 * status: active
 * tags: [store],[state],[zustand]
 *
 * brief: 应用全局状态管理，含 HTTP 轮询与 WebSocket 降级回退
 *
 * details:
 * - 基于 Zustand 的轻量状态管理
 * - HTTP 5s 轮询，支持 WebSocket 降级回退
 * - 管理：任务数据、模型配置、官员统计、UI 状态等
 * - 内置 Pipeline 流程定义（PIPE）
 * - 自动轮询机制（startPolling / stopPolling）
 *
 * dependencies: zustand, ./api
 * exports: useStore, PIPE, PIPE_STATE_IDX, DEPT_COLOR, STATE_LABEL, startPolling, stopPolling
 */
...
export function getPipeStatus(t: Task): PipeStatus[] {
  ...
  return PIPE.map((stage, i) => ({
    ...stage,
    status: (i < stateIdx ? 'done' : i === stateIdx ? 'active' : 'pending') as
      'done' | 'active' | 'pending',
  }));
}
...
export const useStore = create<AppStore>((set, get) => ({
  ...
  handleWSEvent: (_topic, _data) => {
    // 处理 live-status 实时推送
    if (_topic === 'task.status' || _topic === 'task.completed' || _topic === 'task.created') {
      // 收到事件后刷新数据
