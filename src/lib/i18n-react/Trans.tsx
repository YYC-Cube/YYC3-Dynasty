/**
 * file: Trans.tsx
 * description: Trans 组件 — 声明式 ICU 翻译 + JSX 插值（vendor 自 @yyc3/i18n-react，MIT）
 * author: YanYuCloudCube Team
 * version: v1.0.0
 * created: 2026-04-24
 * updated: 2026-07-16
 * status: active
 * tags: [i18n],[react],[component],[vendor]
 *
 * brief: <Trans> 组件 — 声明式 ICU 翻译 + JSX 插值（本地化内嵌）
 *
 * license: MIT
 */
 ...
 */
function interpolateJSX(text: string, components: Record<string, ReactNode>): ReactNode[] {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (comp !== undefined && isValidElement(comp)) {
      ...
      if (content) {
        parts.push(<Fragment key={`trans-${key++}`}>{cloneWithChildren(comp, content)}</Fragment>);
      } else {
