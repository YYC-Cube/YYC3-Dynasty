# YYC³ Dynasty · Figma Make 全链路闭环提示词工程 v1.0

> **三省以治，六部以行** — 中华古制 × AI 协同 × Figma Make 五模型差异化策略
>
> 审核日：2026.06.14 | 基于 6 份源文档去重分拣重排 | 可直接用于 Figma Make 生成
...
核心创新：**首次将 Figma Make 五大AI模型（Claude Sonnet 4.6 / Claude Opus / Gemini 3 Flash / Gemini 3.1 Pro / GPT-5.5）的差异化推理链路特征嵌入提示词工程**，实现"模型选型→初始提示词→迭代提示词→校验闭环"四阶段完整链路。

---

# 第一部分：Figma Make 五模型选型策略

## §1 五模型能力画像与推理特征

基于 Figma 官方文档及 2026 年最新模型基准测试：
...
> **推荐模型**：Claude Sonnet 4.6（平衡效率与品质）
> **复杂度**：中低 | **Credits 预估**：8-12

---

### [Claude Sonnet 4.6] 初始提示词

```
You are a senior UI designer specializing in ancient Chinese imperial aesthetics fused with modern dark-mode dashboards. Create a welcome/launch page for "YYC³ Dynasty" — an AI multi-agent governance platform styled after the Tang Dynasty's "Three Departments and Six Ministries" system.

BACKGROUND:
The page represents "entering the imperial palace." Use the Yingtian Gate (应天门) of Luoyang's Ziwei City as a silhouette backdrop.

FRAME: 520×680px, dark theme.

LAYOUT (top to bottom):
1. TOP SECTION (centered):
   - A 56×56px 3D imperial jade seal (传国玉玺) icon, golden with dragon knob
   - Title "YYC³ Dynasty" in serif font 28px, gold (#d4a843)
   - Subtitle "三省以治 · 六部以行" in serif 14px, secondary gold

2. MIDDLE SECTION (3×2 grid, 12px gap):
   Six entry cards, each containing an icon + ancient title + short description:
   ...
   Hover: border brightens to #d4a843, slight scale up 1.02x

3. BOTTOM SECTION:
   - A golden gradient divider
   - Status text: "紫微城中枢 · 十二朝臣当值"
   - Primary CTA button "奉天承运" (gold gradient bg, serif font 16px)

ANIMATION (describe keyframes in notes):
   - Jade seal drops from top, stamps with red ink spread
...
> **推荐模型**：Claude Sonnet 4.6（结构化数据展示，组件一致性要求高）
...
> **推荐模型**：Claude Opus（最高复杂度的圣旨动效 + 玉玺盖章闭环）
> **复杂度**：极高 | **Credits 预估**：30-50
...
> **推荐模型**：Claude Sonnet 4.6（标准面板 + 复杂交互）
> **复杂度**：中 | **Credits 预估**：12-18
...
> **推荐模型**：Claude Opus（双架构映射 + 桥接动画的高复杂度）
> **复杂度**：高 | **Credits 预估**：22-35
...
> **推荐模型**：Claude Sonnet 4.6（数据可视化 + 监控面板）
> **复杂度**：中 | **Credits 预估**：12-18
